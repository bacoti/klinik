...
const NurseDashboard: React.FC = () => {
  ...
  const mockData = {
    ...
    pendingScreenings: [
      { id: 1, patient: 'Maria Garcia', registeredAt: '09:15 AM', priority: 'normal' },
      { id: 2, patient: 'Robert Chen', registeredAt: '09:30 AM', priority: 'urgent' },
      { id: 3, patient: 'Anna Kowalski', registeredAt: '09:45 AM', priority: 'normal' },
      { id: 4, patient: 'James Wilson', registeredAt: '10:00 AM', priority: 'emergency' }
    ],
    recentVitals: [
      { id: 1, patient: 'Emily Davis', bp: '120/80', temp: '36.5°C', pulse: '72', time: '10 mins ago' },
      { id: 2, patient: 'Michael Brown', bp: '130/85', temp: '37.2°C', pulse: '85', time: '25 mins ago' },
      { id: 3, patient: 'Susan Johnson', bp: '110/70', temp: '36.8°C', pulse: '68', time: '40 mins ago' }
    ]
  };
  ...
  return (
    <DashboardLayout
      ...
      userRole="Nurse"
      userName="Nurse Patricia Martinez"
          ...
          <CardContent>
            <div className="space-y-4">
              {mockData.pendingScreenings.map((screening) => (
                <div key={screening.id} className="flex items-center justify-between p-4 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center space-x-4">
                    <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
                      screening.priority === 'emergency' ? 'bg-red-100' :
                      screening.priority === 'urgent' ? 'bg-orange-100' :
                      'bg-blue-100'
                    }`}>
                      <svg className={`w-6 h-6 ${
                        screening.priority === 'emergency' ? 'text-red-600' :
                        screening.priority === 'urgent' ? 'text-orange-600' :
                        'text-blue-600'
                    ...
                    <div>
                      <p className="font-medium text-gray-900">{screening.patient}</p>
                      <p className="text-sm text-gray-500">Registered at {screening.registeredAt}</p>
                    </div>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    screening.priority === 'emergency' ? 'bg-red-100 text-red-800' :
                    screening.priority === 'urgent' ? 'bg-orange-100 text-orange-800' :
          ...
          <CardContent>
            <div className="space-y-4">
              {mockData.recentVitals.map((vital) => (
                <div key={vital.id} className="p-4 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center justify-between mb-2">
                    <p className="font-medium text-gray-900">{vital.patient}</p>
                    <span className="text-xs text-gray-400">{vital.time}</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-sm">
                    <div className="text-center p-2 bg-red-50 rounded">
                      <p className="text-red-600 font-medium">{vital.bp}</p>
                      <p className="text-gray-500 text-xs">BP</p>
                    </div>
                    <div className="text-center p-2 bg-orange-50 rounded">
                      <p className="text-orange-600 font-medium">{vital.temp}</p>
                      <p className="text-gray-500 text-xs">Temp</p>
                    </div>
                    <div className="text-center p-2 bg-green-50 rounded">
                      <p className="text-green-600 font-medium">{vital.pulse}</p>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <button className="flex flex-col items-center justify-center px-6 py-8 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all duration-200 hover:scale-105">
                ...
                <span className="font-medium">Start Screening</span>
              </button>
              ...
              <button className="flex flex-col items-center justify-center px-6 py-8 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-all duration-200 hover:scale-105">
                ...
                <span className="font-medium">Patient Triage</span>
              </button>
            </div>
          </CardContent>
        </Card>
